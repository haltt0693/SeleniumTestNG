...
import static common.LocatorsCRM.itemCustomerFirst;
...
//            driver.findElement(by).isDisplayed();
    ...
    public void verifyRedirectToCustomerPage() {      // Redirect/ Navigator
        System.out.println("===============================");
        Assert.assertTrue(checkElementExist(titleCustomerPage),"The customer header page is not display");
        Assert.assertEquals(WebUI.getText(titleCustomerPage), "Customers Summary","The customer header page is not correct");
    }
    ...
    public void searchCreateCustomer(String Customer_name){
        ...
        Assert.assertEquals(WebUI.getText(itemCustomerFirst),Customer_name,"Customer_name is incorrect");
    }
...
//        System.out.println("Country: " + WebUI.getText(dropdownCountry));
        ...
        Assert.assertEquals(driver.findElement(inputCompany).getAttribute("value"),Customer_name,"Customer_name is incorrect in detail page");
        WebUI.getValue(inputVATNumber);
        Assert.assertEquals(driver.findElement(inputVATNumber).getAttribute("value"),"11","VATNumber is incorrect");
        WebUI.getValue(inputPhone);
        Assert.assertEquals(driver.findElement(inputPhone).getAttribute("value"),"0987654321","Phone number is incorrect");
        WebUI.getValue(inputWebsite);
        Assert.assertEquals(driver.findElement(inputWebsite).getAttribute("value"),"https://afamily.vn/","Website is incorrect");
//        WebUI.getText(dropdownGroups);
        Assert.assertEquals(WebUI.getText(dropdownGroups),"GroupVIP, VIP","Groups are incorrect");
        WebUI.getTitle(dropdownCurrency);
//        WebUI.getText(dropdownLanguage);
        Assert.assertEquals(WebUI.getText(dropdownLanguage),"Vietnamese","Language is incorrect");
        WebUI.getValue(inputAddress);
        WebUI.getValue(inputCity);
        WebUI.getValue(inputState);
        WebUI.getValue(inputZipCode);
        WebUI.getText(dropdownCountry);

    }
    public int getTotalCus(){
        System.out.println("===============================");
        String totalCusString = driver.findElement(TotalCus).getText();
