...
//        Assert.assertEquals(driver.findElement(errorMessage).getText(), "Invalid email or password", "Content of error massage NOT match.");
        WebUI.assertEquals(WebUI.getText(errorMessage),"Invalid email or password","Content of error massage NOT match.");
    }
...
//        Assert.assertEquals(driver.findElement(errorMessage).getText(), message, "Content of error massage NOT match.");
        WebUI.assertEquals(WebUI.getText(errorMessage),message,"Content of error massage NOT match.");
    }
...
//        String actualMessage2 = driver.findElement(errorMessage1).getText();
        ...
        Assert.assertEquals(WebUI.getText(errorMessage1), "The Password field is required.", "Content of error massage 1 NOT match.");
        Assert.assertEquals(WebUI.getText(errorMessage2), "The Email Address field is required.", "Content of error massage 2 NOT match.");
    ...
    public void verifyLoginFailWithNullFields_ArrayList(int totalNullFields) {
        ...
        for (int i = 1; i <= totalNullFields; i++) {
            ...
            for (int j = 0; j < messageString.size(); j++) {
                if (WebUI.getText(By.xpath("(//div[contains(@class, 'alert-danger')])[" + i + "]")).equals(messageString.get(j))) {
                    check = true;
                    break;
                }
            }
